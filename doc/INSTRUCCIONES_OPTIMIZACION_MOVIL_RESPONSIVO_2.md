...
Usar use_container_width=True en TODAS las llamadas a st.plotly_chart(fig, use_container_width=True).

Habilitar la propiedad config={'responsive': True, 'displayModeBar': False} en Plotly para deshabilitar barras de herramientas pesadas en móviles.

Configurar márgenes compactos: margin=dict(l=10, r=10, t=25, b=25).

Leyendas ubicadas horizontalmente arriba o abajo del gráfico (orientation="h").
...
def crear_grafico_responsivo_sidebar(fig, altura_movil=260):
    """Aplica configuraciones móviles universales a cualquier figura Plotly."""
    fig.update_layout(
        autosize=True,
        height=altura_movil,
...
doc_path = "output_docs/INSTRUCCIONES_OPTIMIZACION_MOVIL_RESPONSIVO.md"
with open(doc_path, "w", encoding="utf-8") as f:
f.write(markdown_content)

print(f"Documento de instrucciones generado exitosamente en: {doc_path}")


```text?code_stdout&code_event_index=1
Traceback (most recent call last):
  File "<xbox-string>", line 103
    markdown_content = """# 📱 GUÍA Y ESPECIFICACIÓN TÉCNICA DE OPTIMIZACIÓN MÓVIL Y RESPONSIVA
...
Documento de instrucciones generado exitosamente en: output_docs/INSTRUCCIONES_OPTIMIZACION_MOVIL_RESPONSIVO.md

Your Markdown file is ready

output_docs/INSTRUCCIONES_OPTIMIZACION_MOVIL_RESPONSIVO
 MD 
Abierto

Se ha generado el archivo con las instrucciones técnicas de arquitectura, diseño y código para que cualquier agente de IA o desarrollador optimice la aplicación para versión móvil y diseño responsivo, unificado bajo la marca SOFTCON-MYS-CONSTRU-WM ("CONSTRUYENDO TU FUTURO").

📋 Resumen del Contenido de las Instrucciones para el Agente
📱 Estrategia Mobile-First & Touch-Friendly:

Configuración inicial con initial_sidebar_state="collapsed" para evitar obstaculizar la pantalla en smartphones.

Botones y controles con área de toque mínima de 44px a 48px de alto para fácil interacción táctil.

🎨 Inyección de CSS Personalizado (Responsive Breakpoints):

Ajuste automático de márgenes (padding), fuentes (h1, h2, h3, p) y formularios al detectar pantallas menores a 768px.

Implementación de desplazamiento horizontal suave (overflow-x: auto) para las tablas complejas del módulo de presupuestos y estimaciones de obra.

📊 Adaptación Responsiva de las 4 Gráficas del Sidebar:

Reutilización de use_container_width=True y configuración config={'responsive': True, 'displayModeBar': False}.

Reducción de tamaños de fuente en ejes y leyendas integradas horizontalmente arriba/abajo para maximizar el área gráfica en pantallas reducidas.

🔀 Reorganización de Layouts:

Reglas de apilamiento vertical y distribución limpia de KPIs (2 columnas máximo en móviles) para evitar desbordamientos.

📋 Checklist de Validación:

Lista de verificación que el agente ejecutará para validar botones de exportación a PDF/Excel, rendimiento con @st.cache_data y teclados numéricos virtuales.