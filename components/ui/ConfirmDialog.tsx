...
export default function ConfirmDialog({
  ...
  useEffect(() => {
    if (isOpen && typeof navigator !== 'undefined' && 'vibrate' in navigator) {
      navigator.vibrate(50); // Light vibration on open
    }
  }, [isOpen]);
    ...
    >
      ...
      >
        <div className="flex items-start gap-4">
          ...
          <button onClick={onCancel} className="flex-shrink-0 text-white/40 hover:text-white/70 p-2 rounded-lg hover:bg-white/10 transition-colors" aria-label="Cerrar diálogo">
            <X className="w-5 h-5" />
          </button>
        </div>
        <div className="flex justify-end gap-3 mt-6">
          <button onClick={onCancel} className="px-4 py-3 rounded-lg bg-white/10 border border-white/20 text-white text-sm hover:bg-white/20 active:scale-95 min-h-[44px] touch-manipulation focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-900">
            {cancelLabel}
          </button>
          <button ref={confirmRef} onClick={onConfirm} className={`px-4 py-3 rounded-lg text-white text-sm font-medium active:scale-95 min-h-[44px] touch-manipulation focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-900 ${buttonBg}`}>
            {confirmLabel}
          </button>
        </div>
      </div>
